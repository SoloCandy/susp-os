...
A **VISUALS** card is pinned to the bottom of the sidebar (collapsible, stays put while the sections above it scroll) and holds every readout/graph in one place, in independently collapsible groups — **DNA MATCH** (only while a Vehicle DNA is linked: each axis's target against the tune's current value), **RIDE · ROLL · DAMPING** (the car's front/rear roll-stiffness split, shaded springs vs ARBs with NAT and target ticks, body roll and ARB share, then front and rear ride Hz, ARB and bump/rebound damping as rows on one shared scale per quantity, every value printed), **DYNAMICS** (a damped step-response chart showing how fast each axle settles and whether it overshoots, with a shaded ±10% settle band, a dashed settle-time marker and a first-neutral-crossing ring per trace — the markers are measured off the drawn curve, so the settle line is literally where the trace enters the band), and **SAG** (the sag vs load chart, when RIDE HEIGHT → CG is active) — so they're visible regardless of which input section is currently open, instead of being scattered across ANTI-ROLL BARS/RIDE/DAMPERS/CHASSIS. What every scale, band, marker and colour means is in [VISUALS.md](docs/VISUALS.md).
