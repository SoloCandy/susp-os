...
| DNA axes (GARAGE → DNA) | each axis's `DNA_AXES` range | Edit a DNA draft, not the tune. APPLY compiles it onto Ride Stiffness, Rear Multiplier, Target Roll Angle, Balance Offset, Rebound ζ, Bump Ratio, Damping Bias, EXIT and ENTRY — see [DNA.md](DNA.md) | As the control each compiles onto |
